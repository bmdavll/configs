...
call s:InitVariable("g:NERDDefaultAlign", "both") "+
call s:InitVariable("g:NERDSexyComs", 1) "+
...
function s:CommentLinesSexy(topline, bottomline)
    if a:topline == a:bottomline
        call s:CommentLines(g:NERDDefaultNesting, "none", a:topline, a:bottomline)
        return
    endif "+
...
function! NERDComment(isVisual, type) range
    ...
    elseif a:type == 'yank'
     ...
     "- execute firstLine .','. lastLine .'call NERDComment('. a:isVisual .', "norm")'
        if g:NERDSexyComs == 1
            execute firstLine .','. lastLine .'call NERDComment('. a:isVisual .', "sexy")'
        else
            execute firstLine .','. lastLine .'call NERDComment('. a:isVisual .', "norm")'
        endif "+
    endif
...
function s:UncommentLinesNormal(topline, bottomline)
    ...
    while currentLine <= a:bottomline
     ...
     "- call setline(currentLine, s:UncommentLineNormal(line))
        call setline(currentLine, substitute(s:UncommentLineNormal(line),'\s\+$','',''))
        let currentLine = currentLine + 1
    endwhile
endfunction
...
"        <silent> "+
nnoremap <silent> <plug>NERDCommenterAltDelims :call <SID>SwitchToAlternativeDelimiters(1)<cr>
...
function s:SexyToggle() "+
    let g:NERDSexyComs = !g:NERDSexyComs
    if g:NERDSexyComs == 1
        call s:NerdEcho("Now using sexy comments", 1)
    else
        call s:NerdEcho("Now using line comments", 1)
    endif
endfunction
nnoremap <silent> <plug>NERDCommenterSexyToggle :call <SID>SexyToggle()<cr>
"+
...
if g:NERDCreateDefaultMappings
 ...
 "- call s:CreateMaps('<plug>NERDCommenterAlignBoth',  ',cb')
    ...
    if !hasmapto('<plug>NERDCommenterSexyToggle', 'n') "+
        nmap ,s <plug>NERDCommenterSexyToggle
    endif
endif
